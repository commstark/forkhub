...
import { sendEmail, toolRejectedEmail } from "@/lib/email"
    ...
    try {
      ...
      if (toolRecord?.creator_id) {
        ...
        if (creatorUser?.email) {
          ...
          sendEmail(
            creatorUser.email,
            `[ForkHub] ${t?.title ?? "Your tool"} was not approved`,
            toolRejectedEmail({
              ...
              notes,
              resubmitSuggestion: `${process.env.NEXTAUTH_URL ?? ""}/review/${params.id}`,
            })
          )
        }
      }
